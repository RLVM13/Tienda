...
async function CategoriaHombre() {
    await fetch("https://fakestoreapi.com/products/category/men's%20clothing")
        ...
        .then((data) => {
            for (let i = 0; i < data.length; i++) {
                if (i==0){
                    principalHombre(data, i);
                }
                relacionadosHombre(data, i);
            }
        });
    return;
}
...
function principalHombre(objeto, indice) {
    let datos1 = `<img src = "${objeto[indice].image}" /></a>`;
    document.getElementById("hombre" + indice).innerHTML = datos1;
    ...
    document.getElementById("hombres" + indice).innerHTML = datos2;

    let datos3 = `<h4>Descripción de Artículo</h4>`;
    datos3 += `<p>"${objeto[indice].description}"</p></div>`;
    document.getElementById("hombress" + indice).innerHTML = datos3;
}

function relacionadosHombre(objeto, indice) {
