...
async function CategoriaJoyeria() {
    await fetch("https://fakestoreapi.com/products/category/jewelery")
        ...
        .then((data) => {
            for (let i = 0; i < data.length; i++) {
                if (i==0){
                    principalJoyeria(data, i);
                }
                relacionadosJoyeria(data, i);
            }
        });
    return;
}
...
function principalJoyeria(objeto, indice) {
    let datos1 = `<img src = "${objeto[indice].image}" /></a>`;
    document.getElementById("joya" + indice).innerHTML = datos1;
    ...
    document.getElementById("joyas" + indice).innerHTML = datos2;

    let datos3 = `<h4>Descripción de Artículo</h4>`;
    datos3 += `<p>"${objeto[indice].description}"</p></div>`;
    document.getElementById("joyass" + indice).innerHTML = datos3;
}

function relacionadosJoyeria(objeto, indice) {
