    ...
    await fetch("https://fakestoreapi.com/products/category/women's%20clothing")
        ...
        .then((data) => {
            for (let i = 0; i < data.length; i++) {
                if (i==0){
                    principalMujer(data, i);
                }
                relacionadosMujer(data, i);
            }
        });
    return;
}
...
function principalMujer(objeto, indice) {
    let datos1 = `<img src = "${objeto[indice].image}" /></a>`;
    document.getElementById("mujer" + indice).innerHTML = datos1;
    ...
    document.getElementById("mujeres" + indice).innerHTML = datos2;

    let datos3 = `<h4>Descripción de Artículo</h4>`;
    datos3 += `<p>"${objeto[indice].description}"</p></div>`;
    document.getElementById("mujeress" + indice).innerHTML = datos3;
}

function relacionadosMujer(objeto, indice) {
